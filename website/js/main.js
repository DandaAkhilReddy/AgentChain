...
// Modern Web3 and Authentication System
class AuthSystem {
    constructor() {
        this.user = null;
        this.wallet = null;
    ...
    async loginWithGoogle() {
        try {
            ...
            if (typeof google === 'undefined') {
                this.showNotification('Google Sign-In is loading...', 'info');
                return;
            }

            google.accounts.id.initialize({
                client_id: '987654321-abcdefghijklmnop.apps.googleusercontent.com', // Demo client ID
                callback: this.handleGoogleCallback.bind(this)
            });

            google.accounts.id.prompt(); // Show the One Tap dialog
            
            // Alternative: Use popup
    ...
    handleGoogleCallback(response) {
        try {
            ...
            this.isAuthenticated = true;
            this.showNotification(`Welcome, ${payload.name}!`, 'success');
            this.updateUIAfterAuth('google');
            
        } catch (error) {
            console.error('Error processing Google callback:', error);
            this.showNotification('Error processing Google login', 'error');
        }
    }
    ...
    launchPlatform() {
        ...
        setTimeout(() => {
            window.location.href = '/app' || './dapp/index.html';
        }, 1500);
    }
